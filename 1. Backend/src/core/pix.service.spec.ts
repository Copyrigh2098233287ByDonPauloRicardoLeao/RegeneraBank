...
describe('PixService (Unit Tests)', () => {
  ...
  describe('executePix (SPI Outbound)', () => {
    ...
    it('deve rejeitar valores negativos ou nulos (Sanitização)', async () => {
      await expect(pixService.executePix(mockAccountId, mockDestKey, -50))
        .rejects
    ...
    it('deve retornar do cache em caso de repetição de IdempotencyKey (Double-Spend Prevention)', async () => {
      ...
      mockIdempotency.get.mockResolvedValueOnce({ body: { status: 'SETTLED_SPI', amountProcessedCents: 1000 } });
      
      const result = await pixService.executePix(mockAccountId, mockDestKey, 10, idempotencyKey);
    ...
    it('deve barrar transações suspeitas bloqueadas pela AML', async () => {
      ...
      await expect(pixService.executePix(mockAccountId, mockDestKey, 9999))
        .rejects
        .toThrow('Transação bloqueada preventivamente pela esteira de segurança.');
    });
    ...
    it('deve realizar PIX com sucesso e acionar o CoreService Atômico', async () => {
      ...
      mockCoreService.executePixAtomic.mockResolvedValueOnce({ senderNewBalance: 400 });

      const result = await pixService.executePix(mockAccountId, mockDestKey, 5, idempotencyKey);
    ...
    it('deve liberar o lock de idempotência em caso de falha de execução', async () => {
      ...
      mockIdempotency.get.mockResolvedValueOnce(null);
      mockCoreService.executePixAtomic.mockRejectedValueOnce(new Error('PostgreSQL Lock Timeout'));

      await expect(pixService.executePix(mockAccountId, mockDestKey, 150, idempotencyKey))
        .rejects
        .toThrow('PostgreSQL Lock Timeout');
      
      expect(mockIdempotency.releaseLock).toHaveBeenCalledWith(idempotencyKey, mockAccountId);
    });
  });
});
