...
export class ComplianceService {
  ...
  async analyzeTransactionRisk(neuralId: string, amountCents: number, originCpf: string): Promise<AmlAnalysisResult> {
    ...
    if (amountCents > 10000000) {
      riskScore += 30;
      this.logger.warn(`[AML ALERTA] Transação atípica de altíssimo valor (R$ ${(amountCents/100).toFixed(2)}) detectada.`);
    }

    // Regras de Congelamento Automático
    if (riskScore >= 90) {
      this.logger.error(`[AML CRÍTICO] Risco Severo (${riskScore}). Conta ${neuralId} será congelada pelo BACEN Circular 3.978.`);
      await this.ledger.freezeAccount(neuralId, 'SUSPEITA_LAVAGEM_DINHEIRO');
      return { isPep, riskScore, action: 'FROZEN', reason: 'COAF_LIMIT_EXCEEDED' };
    } 
    
    if (riskScore >= 60) {
      this.logger.warn(`[AML ALERTA] Risco Moderado (${riskScore}). Enviando para a mesa de operações (Review Manual).`);
      return { isPep, riskScore, action: 'MANUAL_REVIEW', reason: 'PEP_OR_HIGH_VALUE' };
    }
  ...
  async generateTaxDocument(neuralId: string, year: number) {
    this.logger.log(`[TAX COMPLIANCE] Emitindo DARF ${year} para conta ${neuralId}`);
    ...
    try {
      ...
      const [rows] = await this.bigquery.query({ query, params: { neuralId, year } });
      const taxableIncomeCents = rows[0]?.taxable_income || 0;
      
      if (taxableIncomeCents === 0) {
        throw new BadRequestException('Nenhum rendimento tributável detectado para o ano informado.');
      }
      ...
      const pdfBuffer = Buffer.from(`DARF ${year} - Imposto Devido: R$ ${(taxDueCents/100).toFixed(2)}`);
      
      await this.storage.bucket(bucketName).file(fileName).save(pdfBuffer, { 
        metadata: { contentType: 'application/pdf' } 
      });

      const [signedUrl] = await this.storage.bucket(bucketName).file(fileName).getSignedUrl({
        action: 'read',
      ...
      this.logger.error(`[TAX FALHA] Erro na comunicação com BigQuery ou Storage: ${error.message}`);
      throw new BadRequestException('Não foi possível gerar o informe de rendimentos no momento.');
    }
  }
}
