...
export class WebhookService {
  ...
  async processIncomingEvent(payload: WebhookPayload, signatureHeader: string, rawBody: string) {
    ...
    if (isProcessed) {
      this.logger.warn(`[WEBHOOK REPLAY] Evento ${payload.eventId} já foi processado anteriormente. Ignorando.`);
    ...
    try {
      this.logger.log(`[WEBHOOK] Roteando evento autêntico: ${payload.type} (ID: ${payload.eventId})`);

      // 3. Roteamento de Negócio
      switch (payload.type) {
        case 'PIX_RECEIVED':
          // Delega o crédito na conta para a engine transacional do Pix
          await this.pixService.processIncomingPix(payload.data, payload.eventId);
          break;
          
        case 'TRANSFER_CONFIRMED':
          this.logger.log(`[WEBHOOK] Transferência via OpenFinance confirmada E2E: ${payload.data.id}`);
          break;
          
        default:
          this.logger.warn(`[WEBHOOK ALERT] Tipo de evento desconhecido ou não suportado: ${payload.type}`);
      }
      ...
      this.logger.error(`[WEBHOOK CRÍTICO] Falha ao processar evento ${payload.eventId}: ${error.message}`);
      throw error;
    }
  }
  ...
  private verifyHmacSignature(rawBody: string, signatureHeader: string) {
    if (!signatureHeader) {
      throw new UnauthorizedException('Header de assinatura HMAC ausente (x-webhook-signature).');
    }

    const secret = this.config.get<string>('WEBHOOK_SECRET') || 'default-secret-for-dev';
    ...
    if (!isValid) {
      this.logger.error(`[WEBHOOK FRAUDE] Tentativa de spoofing. Assinatura inválida detectada.`);
      throw new UnauthorizedException('Assinatura digital inválida. Chamada rejeitada.');
    }
  }
}
