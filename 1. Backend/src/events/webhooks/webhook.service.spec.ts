...
describe('WebhookService (Events)', () => {
  ...
  describe('processIncomingEvent', () => {
    it('deve rejeitar payload sem eventId', async () => {
      await expect(
        webhookService.processIncomingEvent({ type: 'PIX', timestamp: '123', data: {} } as any, 'sig', 'body')
      ).rejects.toThrow(BadRequestException);
    });

    it('deve rejeitar assinatura HMAC inválida ou vazia', async () => {
      const payload = { eventId: 'evt_1', type: 'PIX', timestamp: '123', data: {} };
      const rawBody = JSON.stringify(payload);

      await expect(
        webhookService.processIncomingEvent(payload, 'wrong-signature', rawBody)
    ...
    it('deve rejeitar e ignorar evento duplicado (Replay Attack) sem chamar o PixService', async () => {
      ...
      const payload = { eventId: 'evt_2', type: 'PIX_RECEIVED', timestamp: '123', data: {} };
      const rawBody = JSON.stringify(payload);
      const signature = generateValidSignature(rawBody);

      const result = await webhookService.processIncomingEvent(payload, signature, rawBody);
    ...
    it('deve processar evento PIX_RECEIVED validado corretamente e delegar ao PixService', async () => {
      const payload = { eventId: 'evt_3', type: 'PIX_RECEIVED', timestamp: '123', data: { amountCents: 1000 } };
      const rawBody = JSON.stringify(payload);
      const signature = generateValidSignature(rawBody);
      ...
      expect(pixServiceMock.processIncomingPix).toHaveBeenCalledWith(payload.data, payload.eventId);
